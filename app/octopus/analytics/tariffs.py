...
def normalize_import_tariffs(
    ...
    result["unit_rate_gbp_per_kwh"] = result["unit_rate_gbp_per_kwh"] / 100
    ...
    result["slot_start"] = result["valid_from"].dt.strftime("%H:%M")

    result["slot_end"] = result["valid_to"].dt.strftime("%H:%M")

    # ==========================================
    # FIND CANONICAL SLOTS
    # ==========================================

    slot_counts = result.groupby(
        [
            "slot_start",
            "slot_end",
        ]
    ...
    canonical_slots = slot_counts[slot_counts == slot_counts.max()].index
