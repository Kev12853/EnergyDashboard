...
try:
    while True:
        ...
        print(snapshot.timestamp.strftime("%Y-%m-%d %H:%M:%S"))
        ...
        print(f"PV Total Power     : {snapshot.pv_power_w} W")

        print(f"PV1 Power          : {snapshot.pv1_power_w} W")

        print(f"PV2 Power          : {snapshot.pv2_power_w} W")
        ...
        print(f"AC Power           : {snapshot.ac_power_w} W")

        print(f"Inverter Status    : {snapshot.inverter_status}")

        print(f"Upload Time        : {snapshot.upload_time}")
